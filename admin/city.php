<?php
	...
	if (isset($_GET['updata'])) {	//修改
		...
		$sql = "update city set name = '". $city_newname . "'where id =" . $city_id;
		$citys = $DB->query($sql);
		...
		$city_name = $_GET['add'];
		$DB = MySql::getInstance();
		$sql = "insert into city values(null,'" . $city_name."')";
		$citys = $DB->query($sql);
...
function update(){
	location.href="city.php?updata="+ cid +"&newname=" + $("#newcityname").val();
}
</script>
			  ...
			  <tr>
			    ...
			    <td>
				    <div class="btn-group">
					  ...
					  <button class="btn" onclick="location.href='city.php?delete=<?php echo $row['id']; ?>'">删除</button>
					</div>
				</td>
			  </tr>
			<?php endwhile; ?>
